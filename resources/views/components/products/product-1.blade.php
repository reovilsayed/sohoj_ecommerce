    ...
    <div class="card shadow-sm border-0 rounded-4 overflow-hidden position-relative h-100">
        ...
        <div class="card-body text-center">
            ...
            <form class="addToCartForm_{{ $product->id }}">
                ...
                <button class="btn btn-sm btn-dark w-100" data-product-id="{{ $product->id }}" type="button" {{ $product->quantity  == 0 ? 'disabled' : '' }}>
                    <i class="fi-rr-shopping-cart me-1"></i>
                    {{ $product->quantity->count() > 0 ? 'Out of Stock' : 'Add to Cart' }}
                </button>
            </form>
        </div>
    </div>
